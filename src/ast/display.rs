...
impl Expression {
    ...
    fn to_latex_inner(&self, parent_prec: u8) -> String {
        match self {
            ...
            | Expression::Variable(_) => latex_basic(self),

            Expression::Unary(op, expr) => latex_unary(op, expr),

            Expression::Binary(op, left, right) => latex_binary(op, left, right, parent_prec),

            Expression::Function(func, args) => latex_function(func, args),

            Expression::Power(base, exp) => latex_power(base, exp),
        }
    }
}

/// LaTeX for scalar and leaf variants: Integer, Float, Rational, Complex,
/// Constant, Variable.
fn latex_basic(expr: &Expression) -> String {
    match expr {
        ...
        Expression::Variable(v) => latex_variable(&v.name),
        _ => unreachable!("latex_basic called on non-basic variant"),
    }
}

/// LaTeX for a variable name, mapping common names to Greek letter commands.
...
/// LaTeX for unary operator expressions.
fn latex_unary(op: &UnaryOp, expr: &Expression) -> String {
    match op {
        UnaryOp::Neg => format!("-{}", expr.to_latex_inner(3)),
        UnaryOp::Not => format!(r"\lnot {}", expr.to_latex_inner(3)),
        UnaryOp::Abs => format!(r"\left|{}\right|", expr.to_latex_inner(0)),
    }
}

/// LaTeX for binary operator expressions.
fn latex_binary(op: &BinaryOp, left: &Expression, right: &Expression, parent_prec: u8) -> String {
    // Division uses \frac
    if let BinaryOp::Div = op {
        return format!(
            r"\frac{{{}}}{{{}}}",
            left.to_latex_inner(0),
            right.to_latex_inner(0)
        );
    }
...
/// LaTeX for function-application expressions.
fn latex_function(func: &Function, args: &[Expression]) -> String {
    // Custom functions
    ...
    // Functions with bespoke LaTeX forms
    match func {
        Function::Sqrt if args.len() == 1 => {
            return format!(r"\sqrt{{{}}}", args[0].to_latex_inner(0));
        }
        Function::Cbrt if args.len() == 1 => {
            return format!(r"\sqrt[3]{{{}}}", args[0].to_latex_inner(0));
        }
        Function::Abs if args.len() == 1 => {
            return format!(r"\left|{}\right|", args[0].to_latex_inner(0));
        }
        Function::Pow if args.len() == 2 => {
            return format!(
                "{}^{{{}}}",
        ...
        Function::Conj if args.len() == 1 => {
            return format!(r"\overline{{{}}}", args[0].to_latex_inner(0));
        }
        Function::Floor if args.len() == 1 => {
            return format!(r"\lfloor {} \rfloor", args[0].to_latex_inner(0));
        }
        Function::Ceil if args.len() == 1 => {
            return format!(r"\lceil {} \rceil", args[0].to_latex_inner(0));
        }
        _ => {}
    }

    let func_name = latex_function_name(func);
    let args_str: Vec<String> = args.iter().map(|a| a.to_latex_inner(0)).collect();
    format!(r"{}\left({}\right)", func_name, args_str.join(", "))
}

/// Map a `Function` variant to its LaTeX command string.
fn latex_function_name(func: &Function) -> &'static str {
    match func {
        Function::Sin => r"\sin",
        Function::Cos => r"\cos",
        Function::Tan => r"\tan",
        ...
        Function::Sqrt => r"\sqrt",
        Function::Cbrt => r"\sqrt[3]",
        Function::Abs => r"\text{abs}",
        ...
        Function::Custom(_) => r"\text{custom}",
    }
}

/// LaTeX for power expressions.
fn latex_power(base: &Expression, exp: &Expression) -> String {
    let base_str = base.to_latex_inner(4);
    let exp_str = exp.to_latex_inner(0);

    let base_needs_braces = matches!(base, Expression::Binary(_, _, _) | Expression::Unary(_, _));
