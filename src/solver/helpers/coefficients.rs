...
use super::detection::{contains_symbol, contains_variable, has_any_variable};
...
/// Port of [`get_polynomial_degree`] over canonical `Arc<Expr>`.
///
...
/// Port of [`extract_quadratic_coefficients`] over canonical `Arc<Expr>`,
/// using exact `BigRational` coefficients.
///
/// Returns `(a, b, c)` such that `expr == a·var² + b·var + c` for
/// polynomial `expr` of degree ≤ 2. Non-polynomial terms and shapes
/// outside the expected form contribute nothing — callers should pre-
/// filter with [`get_polynomial_degree_expr`].
