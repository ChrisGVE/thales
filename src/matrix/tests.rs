...
fn test_matrix_creation() {
    let m =
        MatrixExpr::from_expr_elements(vec![vec![int(1), int(2)], vec![int(3), int(4)]]).unwrap();
...
fn test_addition() {
    ...
    let b =
        MatrixExpr::from_expr_elements(vec![vec![int(5), int(6)], vec![int(7), int(8)]]).unwrap();
...
fn test_inverse_2x2() {
    ...
    let m =
        MatrixExpr::from_expr_elements(vec![vec![int(4), int(7)], vec![int(2), int(6)]]).unwrap();
...
fn test_inverse_singular_matrix() {
    ...
    let m =
        MatrixExpr::from_expr_elements(vec![vec![int(1), int(2)], vec![int(2), int(4)]]).unwrap();
...
fn test_characteristic_polynomial_2x2() {
    ...
    let m =
        MatrixExpr::from_expr_elements(vec![vec![int(2), int(1)], vec![int(1), int(2)]]).unwrap();
...
fn test_eigenvalues_diagonal() {
    ...
    let m =
        MatrixExpr::from_expr_elements(vec![vec![int(5), int(0)], vec![int(0), int(3)]]).unwrap();
...
fn test_lu_decompose_2x2() {
    // A = [[4, 3], [6, 3]]
    let a =
        MatrixExpr::from_expr_elements(vec![vec![int(4), int(3)], vec![int(6), int(3)]]).unwrap();
...
fn test_lu_decompose_identity() {
    ...
    for i in 0..3 {
        ...
        for j in 0..3 {
            ...
            assert!((evaluate(l.get(i, j).unwrap(), &empty).unwrap() - expected_l).abs() < 1e-10);
            assert!((evaluate(u.get(i, j).unwrap(), &empty).unwrap() - expected_u).abs() < 1e-10);
        }
    }
}
...
fn test_solve_system_2x2() {
    ...
    let a =
        MatrixExpr::from_expr_elements(vec![vec![int(2), int(1)], vec![int(1), int(3)]]).unwrap();
...
fn test_solve_system_dimension_mismatch_error() {
    let a = MatrixExpr::identity(2);
    let b = MatrixExpr::from_expr_elements(vec![vec![int(1)], vec![int(2)], vec![int(3)]]).unwrap();
